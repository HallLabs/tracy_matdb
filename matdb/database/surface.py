...
class Surface(Group):
    ...
    Returns:
        surface (np.n darray): an array of atoms objects of length
    ...
    def __init__(self, rattle=0, ran_seed=None, volume_factor=1.0,
                 cov_diag=0.001, min_index=0, name="Surface", **dbargs):
        self.name = name
        self.seeded = True
        dbargs['prefix'] = "D"
        dbargs['cls'] = Surface
        if "Surface" not in dbargs['root']:
            new_root = path.join(dbargs['root'], "Surface")
            if not path.isdir(new_root):
                mkdir(new_root)
            dbargs['root'] = new_root
        super(Surface, self).__init__(**dbargs)
    ...
    def setup(self, rerun=0):
        ...
        """
        super(Surface, self).setup(self._setup_configs, rerun)
        if len(self.sequence) != 0:
            self.index = {}
            self.duids = []
    ...
    def _setup_configs(self, rerun=0):
        """Loops over the surface routine until the desired number of

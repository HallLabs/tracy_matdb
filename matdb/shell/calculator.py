...
"""The Script for the calculator matdb_shell commands. This script is called to
edit the matdb.yml calculator context arguments for different projects and
...
import readline
import numpy as np
import os  # For saving and loading calculators.
import sys
import importlib
...
class ContextShell(Cmd):
    """An Interactive Shell Interface to matdb/calculator.
    The calculator subclass context is accessed as an interface to the
    available groups and matdb.yml calculator option.
    """
    def __init__(self):
        ...
        """
        self.calculator_log = './calculator/calculator.log'
        if (os.path.isfile(self.calculator_log)):
            pass
        else:
            os.system('touch {}'.format(self.calculator_log))
        super(ContextShell, self).__init__(use_ipython=True)
        readline.read_history_file(self.calculator_log)
    ...
    def preloop(self):
        ...
        try:
            ...
            self.config.insert(len(self.config), 'calculator', [])

    def do_recompute(self, args):
        """
    Choose which, calculations to recompute for the scope of the
calculator being edited. Runs on setup if the calculator was previously
defined.
        """
        self.poutput("Editing an existing calculator. Choose a recompute" +
    ...
    def do_calculator(self, name):
        """
        """
        tasks = []
        try:
            group_type = self.config['calculator']['name']
            tasks.append((group_type, self.colorize(
                "Edit {}".format(group_type), 'bold')))
            tasks.append(('n', self.colorize('Change calculator type',
                                             'bold')))
        except KeyError:
            tasks.append(('n', self.colorize('Add New Calculator',
                                             'bold')))
        tasks.append(('r', 'Delete the calculator and Exit.'))
        tasks.append(('q', 'Quit to database list.'))
        selected = self.select(tasks, "Choose a group option: ")
        if(selected == 'n'):
            self._new_group()
        elif(selected == 'r'):
            del self.config['calculator']
    ...
    def _new_group(self):
        ...
        for calc in self.db_calcs:
            tasks.append((calc, self.colorize('Do {}'.format(calc),
                                              'bold')))
        tasks.append(('q', 'Return to Calculator Start.'))
        selected = self.select(tasks, "Choose a group: ")
        if selected == 'q':
            self.poutput('Quitting back to calculator start.')
            self.poutput(self.colorize("TYPE \'calculator\'", 'bold'))
        else:
            self.config['calculator']['name'] = selected
            ruamel.yaml.round_trip_dump(self.config, open('matdb.yml', 'w'),
                                        indent=self.ind,
                                        block_seq_indent=self.bsi)
            self.db_calc = selected
            self.poutput("Entering Calculator Setup.")
            self.do_setup('\n')
...
see a list of some options within the range of this parameter.
    ...
    def do_quit(self, args):
        """Quits the calculator shell."""
        print("\nQuitting Calculator Context to MATDB.\n")
        self.loader.save()
        return -1
...
if __name__ == '__main__':
    prompt = ContextShell()
    message = ('\n Starting matdb calculator...\n')
    prompt.prompt = '(matdb/calculator) > '

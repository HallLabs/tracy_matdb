...
class Tracy(AsyncCalculator):
    ...
    def __init__(self, folder, role=None, notifications=None,
                 ...
                 min_mem=None, ncores=None, max_net_lat=None):
    ...
    def prep_submit(self, folder):
        ...
        self.get_input_dict()
        self.package = {}
        self.package["MatDB ID"] = self.atoms.uuid
        self.package["MatDB Group ID"] = self.atoms.group_uuid
        # package["Source ID"] = Get this from Josh's scripit
        self.package["Source"] = def._get_source()
        self.package["Contract Type"] = self.contract_type
        self.package["Input Dictionary"] = self.input_dict
        self.package["Input Dictionary"]["cryst"] = self._compress_struct(self.atoms)
        self.package["Before fingerprint"] = soap(self.atoms)
        self.package["Contract Priority"] = self.contract_priority
        self.package["eCommerce Priority"] = self.ecommerce
...
class Tracy_QE(Tracy, Qe):
    ...
    def __init__(self, atoms, folder, contr_dir, ran_seed, *args, **kwargs):
        ...
        Qe.__init__(self, atoms, folder, cantr_dir, ran_seed, **QE_input)
        Tracy.__init__(self, **tracy_input)
            ...
            for line in f:
                if "&" in line:
                    ...
                    if "=" in line:
                        ...
                        continue
                    elif k_val is not and key == "k_points":
